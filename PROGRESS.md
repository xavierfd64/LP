...
- A full Playwright regression pass (chart rendering on desktop/mobile, branded headers on all five transaction pages, Staff/Admin chat mounting and permission-gating in all three access states, two-context real-time delivery across Customer/Staff/Admin, and a smoke pass over every earlier update's "must not break" list) was run separately; see the follow-up note below this section once results land, or the commit immediately after this one if a fix was needed.
